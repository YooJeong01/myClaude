...
**예외**: Codex가 OOM이나 usage limit으로 죽었을 때, `Agent` 툴로 `model: "haiku"` 서브에이전트를 띄워
남은 태스크를 이어받게 하는 것은 허용 — 이건 병렬 실행이 아니라 끊긴 Codex의 순차적 대체라 위 근거
(격리 이점 없음, 병렬 이득 없음)가 적용 안 된다. `.agents/workflow.md` §7 참고.
