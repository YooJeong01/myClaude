...
## T37. 스크래핑 범위 정교화 — 우선순위 6 [완료 2026-09-09 → 3단계 분류로 개선, 기존 331→48건 정리]
...
- day6 E2E 시나리오 3개(검색·북마크/캘린더·모아보기) Claude 가 추가 → 전체 E2E 9/9 통과 (563bf94).
- 상세: `artifacts/ai-notes/2026-09-09 day6 야간 진행.md`.

## 후속 (2026-09-09 새벽, 사용자 테스트 피드백)

- **캘린더 진입점** (`16d594e`) — 대시보드/기업분석목록/캘린더 3개 페이지 헤더 상호 네비.
- **T37 3단계 분류로 개선** (`7572f12`) — `classifyRole` → relevant / review / irrelevant.
  영어·약어·프레임워크명 확장. 백엔드·앱·데이터 등 인접 직무는 `review`(저장, 확인 대상).
  `verify-role-filter` 18/18. **기존 `job_postings` 331 → 48 정리** (irrelevant 283 삭제, review 13 유지).
- **URL 파싱 스탠스** — 사용자 OK 확인.
- ⚠️ 캘린더 네비 + 필터 변경 이후 `pnpm build` / `pnpm test:e2e` 는 **호스트 메모리 부족(가용 ~1.2GB)으로
  미실행** — tsc/lint/verify 는 그린. 아침에 재실행 확인 필요.
- 상세: `artifacts/ai-notes/2026-09-09 day6 야간 진행.md`.
