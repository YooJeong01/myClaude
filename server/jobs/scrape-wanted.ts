 ...
 *
 * 실행:
 *   pnpm exec tsx --env-file=.env.local server/jobs/scrape-wanted.ts
 */
import { createAdminClient } from '../supabase/admin';
import { fetchWantedListings } from '../scraping/wanted/client';
...
async function main(): Promise<void> {
  ...
  const postings = await fetchWantedListings();
  ...
  const { inserted, skipped } = await insertCollectedJobPostings(admin, userId, postings, 'scrape_wanted');
