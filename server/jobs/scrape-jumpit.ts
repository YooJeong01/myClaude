 ...
 *
 * 실행:
 *   pnpm exec tsx --env-file=.env.local server/jobs/scrape-jumpit.ts
 */
import { createAdminClient } from '../supabase/admin';
import { fetchJumpitListings } from '../scraping/jumpit/client';
...
async function main(): Promise<void> {
  ...
  const postings = await fetchJumpitListings();
  ...
  const { inserted, skipped } = await insertCollectedJobPostings(admin, userId, postings, 'scrape_jumpit');
